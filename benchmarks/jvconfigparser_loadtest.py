...
def print_jv_configparser():
    ...
    for config_section in ucp.config_dict:
        print('[{}]'.format(config_section))
        for options in ucp.config_dict[config_section]:
...
def print_jv_configparser_compatible():
    cp = ConfigParser(allow_no_value=True)
    ucp = ConfigParser(allow_no_value=True)
    cp.read(r'../default_config')
    ucp.read(r'../user_config')
...
if __name__ == '__main__':
    #print_jv_configparser()
    print_jv_configparser_compatible()
